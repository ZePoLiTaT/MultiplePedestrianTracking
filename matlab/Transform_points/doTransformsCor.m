...
nt_path = strcat( folder, 'NTracks_cor.txt' );

% Ground Truth and NTracks transformed to the Ground Plane file names
GP_gt_cor_file = strcat(folder, 'GP_tracks_cor.mat' );
GP_nt_cor_file = strcat(folder, 'GP_NTracks_cor.txt' );
...
%get the average only in the bottom of the rectangle
gt_feet = gt_ori;
gt_feet(:,:,y) = ceil(gt_ori(:,:,y) + gt_ori(:,:,h)/2);

%set the last 2 columns as the x (col3) y center (col 8)
...
save(GP_gt_cor_file, 'tracks_cor');