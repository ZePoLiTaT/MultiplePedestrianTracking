...
%     mse_dnt = sqrt(sum(sum(dnt.^2)));
    ...
    for t=1:size( naive_track_list,2 )
        ...
        if all( naive_track<=0 )
            if num_gt>1 & ~all( grount_truth_list{2}(1:2,t) <=0 )
                gt_min(:,t) = grount_truth_list{2}(1:2,t);
            else
                gt_min(:,t) = grount_truth_list{1}(1:2,t);
            end
            continue;
        end
        ...
        for gi = 1:num_gt
            ...
            groun_truth = grount_truth_list{gi}(1:2,t);
        ...
        gt_min(:,t) = grount_truth_list{ix}(1:2,t);
